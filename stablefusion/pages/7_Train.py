    ...
    if task == "Dreambooth":
        ...
        with st.form("Train With Dreambooth"):
            ...
            with col1:
                ...
                output_dir_name = st.text_input(label="Name of your Trainned model(folder): ", value="person", help="The output directory name where the model predictions and checkpoints will be written.")
            ...
            with col3:
                ...
                prior_loss_weight_condition = st.selectbox(label="Want to use prior_loss_weight:  ", options=[False, True])
                prior_loss_weight = st.slider(label="Prior Loss Weight: ",value=1.0, max_value=10.0, help="The weight of prior preservation loss.")
                train_text_encoder_condition = st.selectbox("Whether to train the text encoder", options=[True, False])
                train_text_encoder = st.text_input(label="Train Text Incoder", value="fp16", help="the text encoder should be float32 precision.")
            ...
            with col4:
                ...
                lr_warmup_steps = st.number_input(label="Learning Rate, Warmup Steps: ", value=500, step=1, help="Number of steps for the warmup in the lr scheduler.")
            ...
            if submit:
                with st.spinner("Trainning..."):
                    ...
                    train_dreambooth.main(pretrained_model_name_or_path=base_model, revision=revision, output_dir=output_dir_name, with_prior_preservation=prior_loss_weight, seed=seed, resolution=resolution, train_batch_size=train_batch_size, train_text_encoder=train_text_encoder_condition, mixed_precision=mixed_precision, use_8bit_adam=use_8bit_adam, gradient_accumulation_steps=gradient_accumulation_steps, learning_rate=learning_rate, lr_scheduler=lr_scheduler, lr_warmup_steps=lr_warmup_steps, num_class_images=num_class_images, sample_batch_size=sample_batch_size, max_train_steps=max_train_steps, save_interval=save_interval, save_sample_prompt=None)
